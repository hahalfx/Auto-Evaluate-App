...
export function TimingDataDisplay({ timingData, samples }: TimingDataDisplayProps) {
  console.log('TimingDataDisplay received:', { timingData, samples });
  
  const formatTime = (timeStr?: string | null) => {
    if (!timeStr || timeStr === "null" || timeStr === "") return "未记录";
    try {
  ...
  const formatDuration = (ms?: number | null) => {
    if (ms === null || ms === undefined || isNaN(ms)) return "未记录";
  ...
  const calculateAverageTimes = () => {
    ...
    const validTimings = {
      voiceRecognition: timings.filter(t => t.voiceRecognitionTimeMs !== null && t.voiceRecognitionTimeMs !== undefined && !isNaN(t.voiceRecognitionTimeMs)),
      interactionResponse: timings.filter(t => t.interactionResponseTimeMs !== null && t.interactionResponseTimeMs !== undefined && !isNaN(t.interactionResponseTimeMs)),
      ttsResponse: timings.filter(t => t.ttsResponseTimeMs !== null && t.ttsResponseTimeMs !== undefined && !isNaN(t.ttsResponseTimeMs)),
    };
                ...
                return (
                  <div key={sample.id} className="border rounded-lg p-4 bg-gray-50">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant="outline" className="text-gray-500">无数据</Badge>
                    </div>
                  </div>
                );
              }
              ...
              return (
                <div key={sample.id} className="border rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="font-medium">{sample.text}</h4>
                    <Badge variant="outline">样本 #{sample.id}</Badge>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium">交互响应</span>
                      </div>
                      <div className="text-lg font-mono">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium">TTS响应</span>
                      </div>
                      <div className="text-lg font-mono">
