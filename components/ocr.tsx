...
export function OCRVideoComponent() {
  ...
  const frameRateRef = useRef({ lastSendTime: 0, displayLastFrameTime: 0, displayFrames: 0 });
  ...
  const getVideoDevices = useCallback(async () => {
    try {
      ...
      toast({ title: "摄像头错误", description: "无法获取视频设备列表，请检查权限。", variant: "destructive" });
      return [];
    } finally {
  ...
  const initCamera = useCallback(async () => {
    ...
    // 防止重复初始化同一设备
    if (lastInitializedDeviceRef.current === selectedDevice && activeStreamRef.current) {
      return;
    }

    if (initializationPromiseRef.current) {
        try { await initializationPromiseRef.current; } catch (e) {}
    }
    ...
    const promise = (async () => {
        ...
        try {
            ...
            toast({ title: "摄像头初始化失败", description: (error as Error).message, variant: "destructive" });
            throw error;
        } finally {
  ...
  const captureAndSend = useCallback(async () => {
    ...
    if (currentRoi && currentRoi[2] > 0 && currentRoi[3] > 0) {
      ...
      context.drawImage(video, currentRoi[0], currentRoi[1], currentRoi[2], currentRoi[3], 0, 0, currentRoi[2], currentRoi[3]);
    } else {
    ...
    try {
      ...
      const imageBytes = Array.from(imageData.data);
      ...
      toast({ title: "OCR 执行失败", description: String(error), variant: "destructive" });
  ...
  const startCapturing = useCallback(async () => {
    try {
      ...
      toast({ title: "OCR识别启动失败", description: String(error), variant: "destructive" });
    }
  }, [toast]);
  ...
  const clearRoi = useCallback(() => {
    if (isCapturing) {
      toast({ title: "操作无效", description: "请先停止OCR识别再清除ROI。", variant: "destructive" });
      return;
    }
    setRoi(null);
  ...
  const startSelectingROI = useCallback(() => {
    if (isCapturing) {
      toast({ title: "操作无效", description: "请先停止OCR识别再修改ROI。", variant: "destructive" });
      return;
    }
  ...
  useEffect(() => {
    ...
    const loop = () => {
      if (isCapturingRef.current) {
        ...
        if (now - frameRateRef.current.lastSendTime >= ocrIntervalRef.current * 1000) {
  ...
  const drawVisuals = useCallback(() => {
    ...
    if (isSelectingROI && roiStartPoint && isDrawingRef.current) {
      ...
      ctx.lineWidth = 2;
      ctx.setLineDash([5, 5]);
      ctx.strokeRect(roiStartPoint.x, roiStartPoint.y, isDrawingRef.current.x - roiStartPoint.x, isDrawingRef.current.y - roiStartPoint.y);
      ctx.setLineDash([]);
    }
    ...
    if (isCapturing) {
      ctx.fillStyle = "lime";
      ctx.font = "16px Arial";
      ctx.fillText(`Rust 推理: ${lastInferenceTime.toFixed(3)}s`, 10, canvas.height - 10);
    }
  }, [roi, isSelectingROI, roiStartPoint, ocrResults, lastInferenceTime, isCapturing]);
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    if (isCapturing || !isSelectingROI || !roiStartPoint || !canvasRef.current) return;
  ...
  const handleMouseUp = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (w < 10 || h < 10) {
      ...
      const newRoi: [number, number, number, number] = [Math.round(x1), Math.round(y1), Math.round(w), Math.round(h)];
      setRoi(newRoi);
      toast({ title: "ROI区域已设置", description: `区域大小: ${newRoi[2]}x${newRoi[3]}`});
    }
  ...
  return (
    <div className="flex flex-col p-4 rounded-lg border bg-white space-y-2 h-full">
      <div className="flex items-center flex-wrap gap-2">
          ...
          <PopoverContent className="w-80" align="start">
            <div className="grid gap-4">
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <Select value={selectedDevice} onValueChange={setSelectedDevice} disabled={!devicesLoaded || videoDevices.length === 0}>
                  <SelectTrigger><SelectValue placeholder="选择摄像头" /></SelectTrigger>
                  <SelectContent>
                    {videoDevices.map((device) => (<SelectItem value={device.deviceId} key={device.deviceId}>{device.label}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <span>OCR 间隔 (秒):</span>
                <Select value={ocrInterval.toString()} onValueChange={updateOcrInterval} disabled={isCapturing}>
                  <SelectTrigger className="w-32"><SelectValue placeholder="选择间隔" /></SelectTrigger>
                  <SelectContent>
        ...
        <Tooltip>
          <TooltipTrigger asChild><Button variant={"ghost"} size="icon" onClick={startSelectingROI} disabled={isCapturing}><SquareDashedMousePointer className="h-4 w-4" /></Button></TooltipTrigger>
          <TooltipContent><p>选择 ROI 区域</p></TooltipContent>
        </Tooltip>
        <Tooltip>
          <TooltipTrigger asChild><Button variant={"ghost"} size="icon" onClick={clearRoi} disabled={!roi || isCapturing}><Eraser className="h-4 w-4" /></Button></TooltipTrigger>
          <TooltipContent><p>清除 ROI 区域</p></TooltipContent>
        </Tooltip>
        ...
        <div className="flex items-center gap-2">
            {!isCapturing ? (
                <Button onClick={startCapturing} disabled={!devicesLoaded || isInitializing}>
                    <Scan className="h-4 w-4 mr-2" />开始 OCR 识别
                </Button>
            ) : (
        ...
        <div className="flex items-center text-sm text-muted-foreground gap-2">
            <MonitorUp className="h-4 w-4" color={isCapturing ? "hsl(var(--primary))" : "orange"} />
            <span>{isCapturing ? `识别中 (${(1 / ocrInterval).toFixed(1)} FPS)` : "实时预览中"}</span>
        </div>
      </div>

      <div className="flex-grow relative flex justify-center items-center bg-gray-900/50 rounded-lg overflow-hidden">
        <video ref={videoRef} className="max-w-full max-h-full" playsInline autoPlay muted />
        <canvas ref={canvasRef} className="absolute top-0 left-0" onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} />
        {isSelectingROI && (
      ...
      <div className="h-24 overflow-y-auto text-sm pr-2">
        {ocrResults.length > 0 ? (
          ocrResults.map((result, index) => (
            <div key={index} className="flex justify-between items-center p-1.5 rounded hover:bg-gray-100">
              <span className="text-gray-800">{result.text}</span>
              <span className="text-primary font-mono text-xs font-semibold">{result.confidence.toFixed(1)}%</span>
            </div>
          ))
        ) : (
          <div className="text-gray-400 italic pt-2">{isCapturing ? "正在识别或未检测到文字..." : "点击“开始OCR识别”以检测文字"}</div>
        )}
      </div>
    </div>
  );
