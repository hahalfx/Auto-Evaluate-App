...
const samplesSlice = createSlice({
  ...
  reducers: {
    ...
    updateSampleResult: (state, action) => {
      ...
      if (sampleIndex >= 0) {
        state.items[sampleIndex].result = result;
      }
    },
    // 完全替换样本数组(如果需要)
    ...
    deleteSample: (state, action) => {
      state.items = state.items.filter(sample => sample.id !== action.payload);
...
// 导出 actions
export const { setSelectedSamples, updateSampleResult, setSamples, deleteSample } = samplesSlice.actions;

// 定义 selectors
export const selectAllSamples = (state: RootState) => state.samples.items;
export const selectSelectedSampleIds = (state: RootState) => state.samples.selectedIds;
export const selectSampleById = (state: RootState, sampleId: number): TestSample | undefined => 
