...
>("tasks/updateTask", async (taskData: Partial<Task> & { id: number }) => {
...
export const deleteTaskAsync = createAsyncThunk<
  { success: boolean; taskId: number },
  number
>("tasks/deleteTask", async (taskId: number) => {
  ...
  reducers: {
    ...
    updateTaskStatus: (state, action) => {
      ...
      if (taskIndex >= 0) {
        ...
        // If currentTask is the same as the updated task, update it too
        if (state.currentTask && state.currentTask.id === taskId) {
          state.currentTask.task_status = status;
        }
      }
    },
    // 更新任务的机器响应
    updateMachineResponse: (state, action) => {
      ...
      if (taskIndex >= 0) {
        ...
        // If currentTask is the same as the updated task, update it too
        if (state.currentTask && state.currentTask.id === taskId) {
          if (!state.currentTask.machine_response) {
            state.currentTask.machine_response = {};
          }
          state.currentTask.machine_response![sampleId] = response;
        }
      }
    },
    // 更新任务的测试结果
    updateTestResult: (state, action) => {
      ...
      if (taskIndex >= 0) {
        ...
        // If currentTask is the same as the updated task, update it too
        if (state.currentTask && state.currentTask.id === taskId) {
          if (!state.currentTask.test_result) {
            state.currentTask.test_result = {};
          }
          state.currentTask.test_result![sampleId] = result;
        }
      }
    },
    // 添加新任务
    ...
    builder
      ...
      .addCase(deleteTaskAsync.fulfilled, (state, action) => {
        if (action.payload.success) {
          ...
          if (
            state.currentTask &&
            state.currentTask.id === action.payload.taskId
          ) {
